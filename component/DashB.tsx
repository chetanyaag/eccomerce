...
export default function DashB() {
    ...
    const handleAsin =(asindata: SetStateAction<string>)=>{
        setAsin(asindata)
        // console.log(cookies.get('tag'))
        if (cookies.get("tag")=="admin"){
            setTag("shoaib12303-21")
        }else{
            setTag(cookies.get("tag"))
        }
        ...
        <>
            ...
            <Grid container>
                ...
                <Grid item xs={0} sm={0} md={8}>
                    {result==1? <CreateForm onChild={handleChildData} onAsin={handleAsin}></CreateForm>:<CopyForm data={result} asin={asin}  tag1={tag}></CopyForm>}
